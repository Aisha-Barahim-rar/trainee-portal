...
<form method="POST" action="{{ route('company.links.update',$link->ID) }}" enctype="multipart/form-data">
                        @csrf
                        <div class="grid gap-6 mt-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <x-form.input-with-icon-wrapper>
                                    ...
                                    <x-form.select withicon id="type" class="block w-full" name="type">
                                        ...
                                        <option value="Hyperlink" @if($link->type=="Hyperlink") selected @endif>Hyperlink</option>
                                        <option value="File" @if($link->type=="File") selected @endif>File</option>
                                    </x-form.select>
                                </x-form.input-with-icon-wrapper>
                            </div>
                            ...
                            <div class="space-y-2" id="input1">
                                ...
                                <x-form.input-with-icon-wrapper>
                                    ...
                                    <x-form.input withicon id="link" class="block w-full" type="text"
                                        name="link" :value="old('link',$link->link)" placeholder="{{ __('Link') }}" />
                                </x-form.input-with-icon-wrapper>
                            </div>
                            ...
                            <div class="space-y-2" id="input2">
                                ...
                                <input
                                    ...
                                    placeholder="{{ __('File') }}" />
                                <a href="/files/{{ $user->ID }}/{{ $link->link }}" class="text-sm text-gray-600"
                                    target=_blank>{{ $link->link }}</a>
                                <input type="hidden" value="{{ $link->link }}" name="oldFile" />
